...
|-------|----------|---------|
| Git history is coarse | Medium | Seven commits total (`b3b7e25` latest); the mode-simplification, UX-rework and YouTube-format work is largely uncommitted working-tree state. Commit before any risky change. |
| Repo has one missing git object | Medium | `9e8057259b73d7…` (parent of `7fd6d713`, which is in `main`'s ancestry) is absent, so full-history `git log`/`rev-list` fail and auto-maintenance errors on every command. `git fetch origin` should restore it. See §18.5. |
...
| `_probe_youtube.py`, `_e2e_download.py`, `_verify_ui_message.py`, `_shot_*.py`, `_check_*.py` in root | Low | Scratch probe **scripts**; not part of the shipped pipeline. Their captured `*.txt` output logs were removed on 2026-09-11 (§18.1). |
...
### 18.1b Text files removed from the repository root

Thirteen root `*.txt` files plus one scratch directory were inspected and removed in
the same cleanup. **Kept:** `requirements.txt`, `requirements-gui.txt`,
`requirements-local.txt` (all referenced by `README.md`) and
`assets/fonts/README.txt` (the bundled font's SIL OFL attribution).

| File(s) | Verdict | Why |
|---|---|---|
| `_check_err.txt`, `_check_out.txt`, `_check_out4.txt` | **Scratch run log** | Captured stdout/stderr from `_check_youtube_ui.py`. One contains a stale `AssertionError` from a run that was subsequently fixed — actively misleading. |
| `_probe_err.txt`, `_probe_out.txt` | **Scratch run log** | Captured output of `_probe_youtube.py` against a real video. |
| `_pytest_out.txt` | **Scratch run log** | A pytest progress dump. |
| `_shot_out.txt`, `_shot_out4.txt`, `_shot_real_err.txt`, `_shot_real_out.txt`, `_shot_run3.txt` | **Scratch run log** | Captured output of the `_shot_youtube*.py` QML smoke scripts. |
| `combined md files.txt` | **Redundant duplicate** | Verified byte-identical to `FOLLOWUP_REVIEW.md` apart from the 9-line status banner that file later gained. Zero unique content. |
| `improvements.txt` | **Superseded input** | The original CJK-quality brief (Aug 18) that `IMPROVEMENTS_TRIAGE.md` triaged. Every proposal is now implemented, explicitly rejected, or tracked in §18.3. Kept for provenance alongside the archived triage. |
| `_mix_probe/` (dir) | **Scratch test fixture** | A hand-built synthetic bundle tree (`_internal/new{i}.txt` + `_internal/sub/old0-19.txt`) used to verify `test_bundle_is_internally_consistent`. No code references it. |

No code reads any `*.txt` at runtime (verified: no `*.txt` glob in the source tree).
Archived to `docs/archive/root-scratch-2026-09-11/`.
...
### 18.5 Known repository-integrity defect (found 2026-09-11, not yet fixed)
...
`7fd6d713` is in `main`'s ancestry (it is the parent of `9d2a21a`), and **its own
parent `9e80572…` is absent from the object store**. Effects:

- `git log` prints the last seven commits and then dies; history older than `7fd6d713`
  is unreadable.
- `git rev-list --count HEAD` and `git rev-list HEAD` fail outright.
- Every git command emits `geometric-repack` / `commit-graph` maintenance errors,
  because auto-maintenance cannot build a graph over a broken parent link.
...
objects, i.e. a repack that deleted its old pack but never wrote a replacement. The
project log records the `C:` drive reaching `238G/238G used, 0 available` during an
interrupted build on 2026-09-11; a `git gc`/`repack` running in that window would
produce exactly this.

**Fix.** Run `git fetch origin` — the remote is
`https://github.com/njoro1/translation-agent.git` and the missing object should be
restorable if it was ever pushed. Fetch only adds objects, so it is safe.
Then confirm with `git fsck --no-progress`; once the `missing commit` line is gone,
`git gc` is safe again.

**Do not run `git gc --prune=now` before this is fixed** — pruning against a broken
graph risks deleting more objects. Note also that `refs/cline/checkpoints/*` holds 116
refs (from the Cline extension), which is what generates most of the dangling-commit
noise in `git fsck` output.
